...
def compute_derivatives(fields, pts):
    ...
    p_xx = _grad2(p, pts, 0); p_yy = _grad2(p, pts, 1); p_zz = _grad2(p, pts, 2)
...
def compute_losses(
    ...
    p_x, p_y, p_z, p_xx, p_yy, p_zz,
    T_x, T_y, T_z, T_xx, T_yy, T_zz,
    labels, wall_normals, p_out, vx_in, vy_in, vz_in, T_in, p_in_bc, T_w,
    ...
    ppe_rhs = -(rho / 1e5) * (
        vx_x[interior] ** 2 + vy_y[interior] ** 2 + vz_z[interior] ** 2
        + 2 * (vx_y[interior] * vy_x[interior]
               + vx_z[interior] * vz_x[interior]
               + vy_z[interior] * vz_y[interior])
    )
    ppe_lhs = p_xx[interior] + p_yy[interior] + p_zz[interior]
    l_ppe = torch.mean(((ppe_lhs - ppe_rhs) / PPE_SCALE) ** 2)
    ...
    l_inlet_p  = torch.mean((p[inlet]              - p_in_bc) ** 2) / p_var
    ...
    return (
        ...
        l_inlet_vx, l_inlet_vy, l_inlet_vz, l_inlet_T, l_inlet_p, l_outlet_p,
...
class NormalizedPINN(nn.Module):
    ...
    When wall_y1 / wall_y2 are set, appends d1 = y − wall_y1 and
    d2 = y − wall_y2 as extra input features (in_dim = 5) to give the
    network explicit awareness of the two internal dividing walls.
    """
    ...
    def __init__(self, net, coord_mean, coord_std, out_mean, out_std,
        ...
        self.wall_eps = wall_eps   # sigmoid sharpness [m]; ~2 mm gives near-step
...
parser = argparse.ArgumentParser(description="PINN22_4 — GT blade cooling with inlet pressure BC and optional wall-distance features")
...
# Internal dividing walls — signed distances appended as extra input features.
# Set both to -1 (default) to disable; falls back to plain 3D input.
parser.add_argument("--wall-y1", type=float, default=-1.0,
                    help="y-position [m] of the first internal wall (pass 1|2 boundary); -1 = disabled")
parser.add_argument("--wall-y2", type=float, default=-1.0,
                    help="y-position [m] of the second internal wall (pass 2|3 boundary); -1 = disabled")
...
if args.run_path:
    ...
    _name = (f"f{FOLDER}"
             ...
             f"{_wall_tag}"
             f"_s{SEED}")
    RUN_PATH = f"../pinn22_4_runs/{_name}"
...
if USE_WALL_FEATURES:
    print(f"Wall features : ON  wall_y1={WALL_Y1:.4f} m  wall_y2={WALL_Y2:.4f} m  in_dim=5")
else:
    print(f"Wall features : OFF  (--wall-y1/--wall-y2 not set)  in_dim=3")
...
# Compute mean inlet pressure from CFD data at the inlet face (same approach as T_inlet)
_press_all   = np.load(DATA_DIR + "press.npy")
_x_min_press = float(_press_all[:, 0].min())
_press_face  = _press_all[_press_all[:, 0] == _x_min_press]
if INLET_Y_MAX_FRAC < 1.0:
    _y_max_press = float(_press_all[:, 1].max())
    _press_face  = _press_face[_press_face[:, 1] <= INLET_Y_MAX_FRAC * _y_max_press]
P_inlet_bc  = float(_press_face[:, 3].mean()) / 1e5   # Pa → bar
print(f"Inlet P from CFD  : mean={P_inlet_bc:.4f} bar")
...
# ── Extend coord stats with sigmoid pass-indicator features ──
# s_i = sigmoid((y − wall_yi) / WALL_EPS) ≈ 0 below wall, 1 above.
# Normalisation stats are computed empirically over the CFD data.
if USE_WALL_FEATURES:
    _y      = cfd_pts[:, 1]
    ...
    coord_mean_net = torch.cat([coord_mean,
                                 torch.tensor([_s1_mean, _s2_mean], dtype=torch.float64)])
    coord_std_net  = torch.cat([coord_std,
    ...
    coord_mean_net = coord_mean
    coord_std_net  = coord_std
...
if USE_WALL_FEATURES:
    _xr = [float(_bg[:, 0].min()), float(_bg[:, 0].max())]
    _zr = [float(_bg[:, 2].min()), float(_bg[:, 2].max())]
    for _wy in [WALL_Y1, WALL_Y2]:
        ax3d_g.plot(_xr, [_wy, _wy], _zr[:1] * 2, c="orange", lw=1.0, ls="--")
        ax3d_g.plot(_xr, [_wy, _wy], _zr[1:] * 2, c="orange", lw=1.0, ls="--")
...
for _gi, (xlabel, ylabel, xi, yi) in enumerate(
    ...
    if USE_WALL_FEATURES:
        for _wy in [WALL_Y1, WALL_Y2]:
            if yi == 1:    # y on vertical axis → horizontal line
                ax.axhline(_wy, color="orange", lw=1.2, ls="--")
            elif xi == 1:  # y on horizontal axis → vertical line
                ax.axvline(_wy, color="orange", lw=1.2, ls="--")
...
{_sep}
  ...
  Inlet BC         (label=1)  vx,vy,vz,T,p     {n_inlet_train:>8,} pts  ({int(FRAC_INLET *100):2d}%)
...
for epoch in range(EPOCHS):
    ...
    (
        ...
        *derivs, lbls, wall_normals,
        P_OUTLET, vx_inlet_bc, vy_inlet_bc, vz_inlet_bc, T_inlet_bc, P_inlet_bc, T_WALL,
        vx_var, vy_var, vz_var, p_var, T_var,
    )

    if N_SUP > 0:
        sup_fields = model(fixed_sup_pts)
    ...
    l_bc_total  = (l_inlet_vx + l_inlet_vy + l_inlet_vz + l_inlet_T + l_inlet_p + l_outlet_p
    ...
    log = {
        ...
        "bc/inlet_p":     safe_log10(l_inlet_p),
    ...
    print(
        f"\n\n\n\n[{epoch+1:>5}/{EPOCHS}]\n"
        f"PDE   div={l_div.item():.3e}  mom_x={l_mom_x.item():.3e}  mom_y={l_mom_y.item():.3e}  mom_z={l_mom_z.item():.3e}  heat={l_heat.item():.3e}  ppe={l_ppe.item():.3e}  | total={l_pde_total.item():.3e}\n"
        f"BC    inlet_vx={l_inlet_vx.item():.3e}  inlet_vy={l_inlet_vy.item():.3e}  inlet_vz={l_inlet_vz.item():.3e}  inlet_T={l_inlet_T.item():.3e}  inlet_p={l_inlet_p.item():.3e}  outlet_p={l_outlet_p.item():.3e}\n"
