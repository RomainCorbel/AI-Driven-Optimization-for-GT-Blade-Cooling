...
def plot_fields(pts, fields, output_dir="../run_Xavier2", tag=""):
...
DEVICE       = "cuda"
DEBUG        = False
# on a mis 20 000 epochs au lieu de 10 000, 128 hidden dim au lieu de 20, et on a 20 rewarm au lieu de 2 pour le cosine annealing
...
scheduler   = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(opt_model, T_0=2000, T_mult=10, eta_min=LR_MIN)
...
for epoch in range(EPOCHS):
    ...
    if epoch in plot_epochs:
        snap_dir = f"../run_Xavier2/{epoch + 1}_{EPOCHS}"
        with torch.no_grad():
...
torch.save(model.state_dict(),        "../run_Xavier2/pinn_model_v5.pt")
torch.save(weights.detach().cpu(),    "../run_Xavier2/loss_weights_v5.pt")
...
model_inf.load_state_dict(torch.load("../run_Xavier2/pinn_model_v5.pt", weights_only=True, map_location="cpu"))
model_inf.eval()
