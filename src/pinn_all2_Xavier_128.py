...
def plot_fields(pts, fields, output_dir="../run_Xavier_128", tag=""):
...
for epoch in range(EPOCHS):
    ...
    if epoch in plot_epochs:
        snap_dir = f"../run_Xavier_128/{epoch + 1}_{EPOCHS}"
        with torch.no_grad():
...
torch.save(model.state_dict(),        "../run_Xavier_128/pinn_model_v5.pt")
torch.save(weights.detach().cpu(),    "../run_Xavier_128/loss_weights_v5.pt")
...
model_inf.load_state_dict(torch.load("../run_Xavier_128/pinn_model_v5.pt", weights_only=True, map_location="cpu"))
model_inf.eval()
