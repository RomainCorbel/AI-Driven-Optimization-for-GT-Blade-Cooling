...
def get_loss(
    ...
    labels,
    p_outlet,
    vx_inlet, vy_inlet, vz_inlet, T_inlet,
    T_wall,
):
    # ── INTERIOR: PDE residuals (label == 0) ─────────────────────────
    interior = labels == 0
    ...
    loss_momentum_x = torch.mean(
        (
            vx[interior] * vx_x[interior]
            + vy[interior] * vx_y[interior]
            + vz[interior] * vx_z[interior]
            + (10**5 / rho) * p_x[interior]
            - (mu[interior] / rho)
            * (vx_xx[interior] + vx_yy[interior] + vx_zz[interior])
        )
        ** 2
    )
    loss_momentum_y = torch.mean(
        (
            vx[interior] * vy_x[interior]
            + vy[interior] * vy_y[interior]
            + vz[interior] * vy_z[interior]
            + (10**5 / rho) * p_y[interior]
            - (mu[interior] / rho)
            * (vy_xx[interior] + vy_yy[interior] + vy_zz[interior])
        )
        ** 2
    )
    loss_momentum_z = torch.mean(
        (
            vx[interior] * vz_x[interior]
            + vy[interior] * vz_y[interior]
            + vz[interior] * vz_z[interior]
            + (10**5 / rho) * p_z[interior]
            - (mu[interior] / rho)
            * (vz_xx[interior] + vz_yy[interior] + vz_zz[interior])
        )
        ** 2
    )
    ...
    # ── INLET BC: Dirichlet on velocity and temperature (label == 1) ──
    inlet = labels == 1
    ...
    loss_inlet_T  = torch.mean((T[inlet]  - T_inlet)  ** 2) / 10**6

    # ── OUTLET BC: Dirichlet on avg static pressure (label == 2) ─────
    outlet = labels == 2

    loss_outlet_p = torch.mean((p[outlet] - p_outlet) ** 2)

    # ── WALL BC: no-slip + temperature (label == 3) ───────────────────
    wall = labels == 3

    loss_wall_vx = torch.mean(vx[wall] ** 2)
    loss_wall_vy = torch.mean(vy[wall] ** 2)
    loss_wall_vz = torch.mean(vz[wall] ** 2)
    loss_wall_T  = torch.mean((T[wall] - T_wall) ** 2) / 10**6
