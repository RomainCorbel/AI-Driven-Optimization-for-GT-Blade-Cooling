...
def plot_fields(pts, fields, output_dir="../run", tag=""):
...
for epoch in range(EPOCHS):
    ...
    if epoch in plot_epochs:
        snap_dir = f"../run/{epoch + 1}_{EPOCHS}"
        with torch.no_grad():
...
torch.save(model.state_dict(),        "../run/pinn_model_v5.pt")
torch.save(weights.detach().cpu(),    "../run/loss_weights_v5.pt")
...
model_inf.load_state_dict(torch.load("../run/pinn_model_v5.pt", weights_only=True, map_location="cpu"))
model_inf.eval()
