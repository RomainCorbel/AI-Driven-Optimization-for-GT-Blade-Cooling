...
def get_values_and_derivatives(fields, points):
    ...
    vx_y = grad_vx[:, 1:2]
    vx_z = grad_vx[:, 2:3]
    # Diagonal Hessian: differentiate each partial w.r.t. its own coordinate only
    ...
    # --- T ---
    # T = fields[:,4:5] * 1000, so grad_T already carries the ×1000 factor — no extra scaling needed.
