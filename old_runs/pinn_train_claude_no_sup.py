...
# PDE loss warmup: ramp pde_weight from 0 → 1 over this many epochs.
# Lets BC + supervised losses anchor all fields before physics takes over.
# PDE_WARMUP_EPOCHS = 2000
# let s try without that to see if it is important:
PDE_WARMUP_EPOCHS = 1
N_TEST  = 10_000
N_TRAIN = 5_000
...
for epoch in range(EPOCHS):
    ...
    # ── PDE warmup weight ────────────────────────────────────
    # Ramps 0 → 1 over PDE_WARMUP_EPOCHS.
    # During early training BC + supervised losses anchor all fields;
    # physics is introduced gradually once the network is in the right ballpark.
    pde_weight = min(1.0, epoch / max(PDE_WARMUP_EPOCHS, 1))
    ...
    # PDE terms get ramped weight; BC + supervised are always fully active
    pde_terms  = pde_weight * torch.sum(weights[:N_PDE]  * all_losses[:N_PDE])
    rest_terms = torch.sum(weights[N_PDE:] * all_losses[N_PDE:])
    l_weighted = pde_terms + rest_terms - weights.sum()
