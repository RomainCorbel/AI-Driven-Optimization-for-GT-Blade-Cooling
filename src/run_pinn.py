...
parser = argparse.ArgumentParser(
    description="PINN27 — multi-DP parametric training with cached geometry pools")

parser.add_argument("--run-path",   default=None,     help="Output dir (auto-derived if omitted)")
parser.add_argument("--project",    default="PINN27", help="W&B project name")
...
# 0.70 / 0.33 match dp00 and apply to all DPs (same general channel shape).
...
parser.add_argument("--dps", nargs="+", default=None, metavar="DP",
                    help="Subset of DPs to use, e.g. --dps dp00 dp03 dp49. "
                         "Omit to use all available folders in preProcessedData/With_T/.")
...
def load_dp(cfg, args):
    ...
    outlet_frac = args.outlet_y_min_frac
    inlet_frac  = args.inlet_y_max_frac
    ...
    # Pressure scale = actual std of CFD pressure field (ensures p_nd has O(1) range).
    # P_DYN = rho*V² can be << actual viscous pressure drop, giving p_var >> 1 and
    # collapsing the supervision gradient for high-friction DPs.
    P_SCALE = float(cfd_p.std().clamp(min=1.0))
    print(f"    V_IN={V_IN:.3f} m/s  P_REF={P_REF:.0f} Pa  P_DYN={P_DYN:.1f} Pa"
          f"  P_SCALE={P_SCALE:.1f} Pa  DELTA_T={delta_t:.2f} K  ν/V_IN={visc_coeff:.2e} m")
    ...
    # ── STL mesh and pools (with disk cache) ──────────────────
    stl_buffer_mm = 420.0
    ...
    pool_dir  = os.path.join("pools", folder)
    os.makedirs(pool_dir, exist_ok=True)
    pool_path = os.path.join(pool_dir,
    ...
    if os.path.exists(pool_path): ### will need less memory if just need to load
        ...
        wall_pool = _saved["wall_pool"]
        _n_uni    = int(_saved["n_uni"])
        _n_horiz  = int(_saved["n_horiz"])
    else:
        ...
        del mesh
        _tmp_path = pool_path + f".tmp{os.getpid()}"
        torch.save({
            "vol_pool":  vol_pool,
            "wall_pool": wall_pool,
            "n_uni":     _n_uni,
        ...
        os.replace(_tmp_path, pool_path)  # atomic on Linux
        print(f"    Cached pools → {os.path.basename(pool_path)}")

    # ── Pool / CFD alignment check ─────────────────────────────
    pool_phys = vol_pool.clone()
    pool_phys[:, 0] -= stl_buffer_mm
    pool_phys /= 1000.0
    cfd_x0, cfd_x1 = float(cfd_pts[:, 0].min()), float(cfd_pts[:, 0].max())
    cfd_y0, cfd_y1 = float(cfd_pts[:, 1].min()), float(cfd_pts[:, 1].max())
    cfd_z0, cfd_z1 = float(cfd_pts[:, 2].min()), float(cfd_pts[:, 2].max())
    pol_x0, pol_x1 = float(pool_phys[:, 0].min()), float(pool_phys[:, 0].max())
    pol_y0, pol_y1 = float(pool_phys[:, 1].min()), float(pool_phys[:, 1].max())
    pol_z0, pol_z1 = float(pool_phys[:, 2].min()), float(pool_phys[:, 2].max())
    print(f"    [align] CFD  x=[{cfd_x0:.3f}, {cfd_x1:.3f}]"
          f"  y=[{cfd_y0:.3f}, {cfd_y1:.3f}]  z=[{cfd_z0:.4f}, {cfd_z1:.4f}] m")
    print(f"    [align] Pool x=[{pol_x0:.3f}, {pol_x1:.3f}]"
          f"  y=[{pol_y0:.3f}, {pol_y1:.3f}]  z=[{pol_z0:.4f}, {pol_z1:.4f}] m")
    x_ok = abs(pol_x0 - cfd_x0) < 0.01 and abs(pol_x1 - cfd_x1) < 0.01
    y_ok = abs(pol_y0 - cfd_y0) < 0.01 and abs(pol_y1 - cfd_y1) < 0.01
    z_ok = abs(pol_z0 - cfd_z0) < 0.005 and abs(pol_z1 - cfd_z1) < 0.005
    if x_ok and y_ok and z_ok:
        print(f"    [align] OK — pool and CFD extents match")
    else:
        print(f"    [align] WARNING — pool/CFD mismatch!"
              f"  dx=[{pol_x0-cfd_x0:+.3f}, {pol_x1-cfd_x1:+.3f}]"
              f"  dy=[{pol_y0-cfd_y0:+.3f}, {pol_y1-cfd_y1:+.3f}]"
              f"  dz=[{pol_z0-cfd_z0:+.4f}, {pol_z1-cfd_z1:+.4f}] m")
    del pool_phys
    ...
    return {
        ...
        "V_IN": V_IN, "P_DYN": P_SCALE, "P_REF": P_REF,
        ...
        # split — train_pts/vx/etc. not stored: they're large (~630 MB/DP) and
        # only needed during this function for variance computation and sup selection.
...
if dp_filter:
    unknown = dp_filter - {c["folder"] for c in DP_CONFIGS}
    if unknown:
...
if args.run_path:
    ...
    # Each SLURM array job gets its own directory; hyperparams are logged in W&B
    _sweep_short = os.environ.get("WANDB_SWEEP_ID", "sweep")[-8:]
    _trial_uid   = os.environ.get("SLURM_ARRAY_TASK_ID", str(os.getpid()))
    RUN_PATH = f"../pinn27_sweep_runs/{_sweep_short}/trial_{_trial_uid}"
else:
    _n_avail = sum(
        1 for c in DP_CONFIGS
        if (not dp_filter or c["folder"] in dp_filter)
        and os.path.isdir(f"./preProcessedData/With_T/{c['folder']}/")
    )
    RUN_PATH = (f"../pinn27_runs/"
                f"n{_n_avail}_h{HIDDEN_DIM}_l{N_LAYERS}"
...
if dp_filter:
    print(f"\nLoading requested DPs: {sorted(dp_filter)}")
else:
...
_param_mean_ext = torch.zeros(N_PARAMS, dtype=torch.float64, device='cpu')
_param_std_ext  = torch.ones( N_PARAMS, dtype=torch.float64, device='cpu')
...
n_global_pts = sum(dp["cfd_pts_nd"].shape[0] for dp in dps)
del all_pts_nd, all_fields_nd, _y_all, _s1, _s2  # free ~28 GB cat copies

# Free full CFD arrays — no longer needed after global stats; inference reloads from disk
for dp in dps:
    del dp["cfd_pts_nd"], dp["cfd_fields_nd"], dp["train_idx"]

coord_mean_net = torch.cat([coord_mean, _param_mean_ext,
                             torch.tensor([_s1_mean, _s2_mean], dtype=torch.float64, device='cpu')])
coord_std_net  = torch.cat([coord_std,  _param_std_ext,
                             torch.tensor([_s1_std,  _s2_std],  dtype=torch.float64, device='cpu')])
...
print(f"\nGlobal stats  (from {N_DPS} DPs, {n_global_pts:,} pts total)")
...
# ── Sweep override — apply W&B controller's hyperparameters if running as agent ──
if IS_SWEEP:
    _sc = wandb.config
...
for epoch in range(EPOCHS):
    ...
    for dp in dps:
        ...
        if dp["n_sup"] > 0:
            sup_f    = model(with_params(dp["sup_pts"].to(DEVICE), dp["params_nd"]))
            ctx      = dp["dp_ctx"]
        ...
        # Accumulate gradients from all DPs before a single optimizer step
        l_total.backward()
        ...
        model.eval()
        with torch.no_grad():
            pred = model(with_params(dp["test_pts"].to(DEVICE), dp["params_nd"]))
    ...
    # One optimizer step using gradients accumulated from all DPs
    opt_model.step()
    scheduler.step()
    ...
    if epoch in plot_epochs:
        ...
        for dp in snap_dps:
            ...
            model.eval()
            with torch.no_grad():
                pred_snap = model(with_params(dp["snap_pts"].to(DEVICE), dp["params_nd"]))
            plot_fields(
...
for dp in dps:
    ...
    # Reload CFD data from disk (freed earlier to save memory during training)
    data_dir = f"./preProcessedData/With_T/{dp['folder']}/"
    _vx_raw  = np.load(data_dir + "vel_x.npy")
    ...
    with torch.no_grad():
        pred_all = model_inf(with_params(cfd_pts_cpu, dp["params_nd"].cpu()))
